...
(function (factory) {
    ...
    class TalkSessionFactory {
        async getConf(config) {
            ...
            return network_1.getBookingData(bookingStream, config);
        }
        async getCheckin(userId, config) {
            let checkinStream;
            const checkinCrypto = await crypto_1.newCryptoStore(config.locoPEMPublicKey);
            try {
            ...
            return network_1.getCheckinData(checkinStream, config, userId);
        }
        async connect(userId, config) {
            ...
            const locoStream = new network_1.LocoSecureLayer(await NetSocket.createTCPSocket({
                ...
                keepAlive: true,
            }), await crypto_1.newCryptoStore(config.locoPEMPublicKey));
