...
import { KnownChatType, DELETED_MESSAGE_OFFSET } from '../../chat/chat-type.js';
...
import { TalkChatData } from '../chat/index.js';
...
class TalkChannelHandler {
    ...
    _msgHandler(msgData, parentCtx) {
        ...
        this._callEvent(parentCtx, 'chat', new TalkChatData(chatLog), this._channel);
    ...
    _msgDeleteHandler(data, parentCtx) {
        ...
        if (chatLog.type !== KnownChatType.FEED)
            return;
        const feed = feedFromChat(chatLog);
        if (feed.feedType !== KnownFeedType.DELETE_TO_ALL)
            return;
    ...
    pushReceived(method, data, parentCtx) {
        switch (method) {
            case 'MSG':
                this._msgHandler(data, parentCtx);
                break;
            case 'FEED':
                this._feedHandler(data, parentCtx);
                break;
            case 'DECUNREAD':
                this._chatReadHandler(data, parentCtx);
                break;
            case 'CHGMETA':
                this._metaChangeHandler(data, parentCtx);
                break;
            case 'DELMEM':
        ...
        switch (method) {
            ...
            case 'SYNCJOIN': {
                ...
                this._updater.addChannel({ channelId: joinData.c }).then((res) => {
                    if (!res.success)
                        return;
                    this._callEvent(parentCtx, 'channel_join', res.result);
                });
                break;
            }
        }
...
export { TalkChannelHandler, TalkChannelListHandler };
